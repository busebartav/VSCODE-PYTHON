...
while True:
    ogrenciAdi = input("Aradığınız öğrenci adını giriniz, çıkmak için ç : ")
    if ogrenciAdi=="ç":
        break
    for i in range(len(ogrenciListesi)):
        if ogrenciAdi == ogrenciListesi[i]:
            print(f"{ogrenciAdi} listenin {i+1}. sırasındaki öğrencisidir.")
            break
    else:
...
ogrenciListesi = ["ali", "kemal", "mehmet", "mustafa"]
while True:
    ogrenciAdi=input("Aradığınız öğrenci:")
    if ogrenciAdi=="çık":
        break
    for i in range(len(ogrenciListesi)):
        ...
        print(f"sınıfra {ogrenciAdi} isimli öğrenci yoktur.")
"""
# ödev →
"""
rakamlar = [6, 2, 7, 3, 5]
